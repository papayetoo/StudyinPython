...
def permutation(numbers=[], r = 0, depth = 0):
    ...
    for i in range(depth, len(numbers)):
        ...
        numbers[i], numbers[depth] = numbers[depth], numbers[i]
# 다음 순열
def next_permutation(nums = [])->bool:
    n = len(nums) - 1
    i = n
    # 마지막 순열인지 검사하는 부분
    while i > 0 and nums[i - 1] >= nums[i]: # 내림차순인지 확인 및 마지막 순열인지 확인.
        i -= 1
    if i == 0:
        return False
    # 마지막 순열인지 검사하는 부분 종료.
    ...
    while nums[i - 1] >= nums[j]:
        ...
        # j 는 감소하지 않음.
        j -= 1
    nums[i - 1], nums[j] = nums[j], nums[i - 1] # 2, 4, 3, 1 로 변경됨.
    while i < j : # i = 1, j = 3 이므로 while 루프 돌지 않음.
        nums[i], nums[j] = nums[j], nums[i] # 2, 1, 3, 4 로 변
        i += 1
        j -= 1
    return  True
